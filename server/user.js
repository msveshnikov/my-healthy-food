...
const userRoutes = (app) => {
    app.post('/api/auth/signup', [
        ...
        body('password').if(body('credential').notExists()).isLength({ min: 6 }).withMessage('Password must be at least 6 characters long')
    ], async (req, res) => {
    ...
    app.put('/api/profile', authenticateToken, [
        ...
        body('researchPreferences.aiAssistance').optional().isIn(['minimal', 'moderate', 'full']),
        ...
        try {
            const { firstName, lastName, preferences, presentationSettings, researchPreferences } = req.body;
            const updateFields = {};
            if (firstName !== undefined) updateFields.firstName = firstName;
            if (lastName !== undefined) updateFields.lastName = lastName;
            if (preferences !== undefined) updateFields.preferences = preferences;
            if (presentationSettings !== undefined) updateFields.presentationSettings = presentationSettings;
            if (researchPreferences !== undefined) updateFields.researchPreferences = researchPreferences;
    ...
    app.post('/api/auth/reset-password', [
        ...
        try {
            ...
            if (!user.emailVerified) {
                return res.status(400).json({ error: 'Email address is not verified. Please verify your email first.' });
            }
